...
describe("Adding big numbers, should throw error", () => {
    ...
    it("#1 interaction call from user A", function(done){
        const interactionA = shell.exec(`${runGlowPath} interact-cli ${contractPath}  ${address} Jan A ${contractParams}  " " `, {silent: silent}).stdout
    ...
    it("#1 interaction call from user B", function(done){
        const interactionB = shell.exec(`${runGlowPath} interact-cli ${contractPath}  ${address} "Marcin" "B" ${contractParams} " " `, {silent: silent}).stdout
