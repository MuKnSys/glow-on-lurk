...
const contractParams = config.contractParamsBig
const contractPtcps = config.contractPtcps
const silent = config.silentMode
var server = null
var address = null

describe("Adding big numbers, should throw error", () => {
    ...
    it("#1 interaction call from user A, addition", function(done){
        const interactionA = shell.exec(`cd ; ${runGlowPath} interact-cli ${contractPath}  ${address} Jan A ${contractParams}  " " `, {silent: silent}).stdout
        expect(interactionA.slice(-10, -1), "Cannot interact with consenus").to.equal('Call sent')
        done()
    })
    it("#1 interaction call from user B, addition", function(done){
        const interactionB = shell.exec(`cd ; ${runGlowPath} interact-cli ${contractPath}  ${address} "Marcin" "B" ${contractParams} " " `, {silent: silent}).stdout
        expect(interactionB.slice(-10, -1), "Cannot interact with consenus").to.equal('Call sent')
        done()
    })
    it("#2 interaction call from user A, addition", function(done){
        const interactionA = shell.exec(`cd ; ${runGlowPath} interact-cli ${contractPath}  ${address} "Jan" "A" ${contractParams}  " " `, {silent: silent}).stdout
        expect(interactionA.slice(-10, -1), "Cannot interact with consenus").to.equal('Call sent')
        done()
    })
    it('checking result of addition', function(done) {
        chai.request('localhost:3000')
        .get('/state')
        .end(function (err, res){
            expect(err).to.exist;
    ...
    //TODO change the result check, error should exist while trying to interact
