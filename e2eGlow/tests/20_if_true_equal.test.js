...
const contractPath = `${config.dapssDir}/if_eq.glow`
...
describe("Checking 'if', with equal", () => {
    ...
    it("#1 interaction call from user A, if statement with equal", function(done){
    ...
    it("#1 interaction call from user B, if statement with equal", function(done){
    ...
    it("#2 interaction call from user A, if statement with equal", function(done){
        const interactionA = shell.exec(`cd ; ${runGlowPath} interact-cli ${contractPath}  ${address} "Jan" "A" ${contractParams}  "" `, {silent : silent})
        expect(interactionA.stdout.slice(-10, -1), `Cannot interact with consenus: ${interactionA.stderr}`).to.equal('Call sent')
        done()
    })
    ...
    it('checking results', function(done) {
        chai.request('localhost:3000')
        .get('/state')
        .end(function (err, res){
            expect(err).to.be.null;
