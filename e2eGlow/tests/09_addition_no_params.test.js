...
const contractPath = `${config.dapssDir}/addingv2.glow`
...
describe("Adding, no params", () => {
    ...
    it("#1 interaction call from user A, addition no parameters", function(done){
    ...
    it("#1 interaction call from user B, addition no parameters", function(done){
    ...
    it("#2 interaction call from user B, addition no parameters", function(done){
        const interactionB = shell.exec(`cd ; ${runGlowPath} interact-cli ${contractPath}  ${address} "Marcin" "B" ${contractParams} "" `, {silent : silent})
        expect(interactionB.stdout.slice(-10, -1), `Cannot interact with consenus: ${interactionB.stderr}`).to.equal('Call sent')
        done()
    })
