...
describe("Transfering founds between two participants and consensus", () => {
    ...
    it("#3 interaction call from user A, deposit", function(done){
        const interactionB = shell.exec(`${runGlowPath} interact-cli ${contractPath}  ${address} "Jan" "A" ${contractParams}  "" `, {silent :silent})
        expect(interactionB.stdout, `Cannot interact with consenus: ${interactionB.stderr}`).to.equal('Call sent\n')
        done()
    })
    ...
    it("#4 interaction call from user A, withdraw", function(done){
        const interactionA = shell.exec(`${runGlowPath} interact-cli ${contractPath}  ${address} "Jan" "A" ${contractParams}  "" `, {silent : silent})
        expect(interactionA.stdout, `Cannot interact with consenus: ${interactionA.stderr}`).to.equal('Call sent\n')
        done()
    })
