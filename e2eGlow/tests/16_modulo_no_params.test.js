...
const contractPath = `${config.dapssDir}/modulov2.glow`
...
describe("Modulo, no parameters", () => {
    ...
    it("#1 interaction call from user A, modulo", function(done){
    ...
    it("#1 interaction call from user B, modulo", function(done){
    ...
    it("#2 interaction call from user A, modulo", function(done){
        const interactionA = shell.exec(`${runGlowPath} interact-cli ${contractPath}  ${address} Jan A ${contractParams}  "" `, {silent : silent})
        expect(interactionA.stdout.slice(-10, -1), `Cannot interact with consenus: ${interactionA.stderr}`).to.equal('Call sent')
        done()
    })
    it("#2 interaction call from user B, modulo", function(done){
        const interactionB = shell.exec(`${runGlowPath} interact-cli ${contractPath}  ${address} Marcin B ${contractParams}  "" `, {silent : silent})
        expect(interactionB.stdout.slice(-10, -1), `Cannot interact with consenus: ${interactionB.stderr}`).to.equal('Call sent')
        done()
    })
    it('checking result of interaction', function(done) {
        ...
        .end(function (err, res){
            ...
            var balanceA = 101
            var balanceB = 199
            expect(JSON.stringify(result), "Incorrect result values").to.equal(`[{"_balance":${balanceA}},{"_balance":${balanceB}}]`)
            done()
        })
    })
